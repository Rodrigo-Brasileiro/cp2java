   ...
   public static void consultar(){
    ...
    try {
        ...
        if (rs.next()) {
            ...
            // Se o valor de horas for negativo, significa que a estadia durou mais de um dia
            if (horasPassadas < 0) {
                horasPassadas = 24 + horasPassadas;  // Ajuste para considerar um dia completo
            }

            // Se a diferença for zero horas, cobra 15 reais por padrão
            if (horasPassadas == 0 && minutosPassados == 0) {
                horasPassadas = 1; // Cobra uma hora mínima se não houver diferença
            }

            // Arredonda para cima se houver minutos extras (garantir que minutos adicionais contam como mais uma hora)
